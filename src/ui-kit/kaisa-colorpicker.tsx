...
import {
  ...
  normalizeHex,
  type Hsv,
} from '@/ui-components/colorpicker/color-utils';

export type KaisaColorpickerSize = 'sm' | 'md' | 'lg';

export type KaisaColorpickerProps = {
  value?: string | null;
...
export function KaisaColorpicker({
  value,
  defaultValue = '#1a1a18',
  onChange,
  ...
  const [hsv, setHsv] = useState<Hsv>(() => getHsvFromColor(defaultValue ?? '#1a1a18', {h: 30, s: 8, v: 10}));
  ...
  return (
    <div
      ...
      className={joinClasses(
        'kaisa-colorpicker',
        `kaisa-colorpicker--${uiSize}`,
        open && 'kaisa-colorpicker--open',
        disabled && 'kaisa-colorpicker--disabled',
        readOnly && 'kaisa-colorpicker--readonly',
        invalid && 'kaisa-colorpicker--invalid',
        className,
      )}
    >
      <div className="kaisa-colorpicker__control">
        <button
          type="button"
          className="kaisa-colorpicker__swatch"
        ...
        <div className="kaisa-colorpicker__actions">
          {clearable && selected && !disabled && !readOnly ? (
            <button
              type="button"
              className="kaisa-colorpicker__clear"
              onClick={handleClear}
              aria-label="색상 지우기"
            >
              ×
          ...
          <button
            type="button"
            className="kaisa-colorpicker__trigger"
            onClick={() => (open ? closePopover() : openPopover())}
            disabled={disabled || readOnly}
      ...
      {open ? (
        <div
          id={`${inputId}-palette`}
          className="kaisa-colorpicker__popover"
          role="dialog"
          aria-label="색상 선택"
        >
          <div
            ref={svRef}
            className="kaisa-colorpicker__sv"
            style={{backgroundColor: hsvToHex({h: hsv.h, s: 100, v: 100})}}
            onPointerDown={handleSvPointerDown}
          >
            <div className="kaisa-colorpicker__sv-white" />
            <div className="kaisa-colorpicker__sv-black" />
            <span
              className="kaisa-colorpicker__cursor"
              style={{
                left: `${hsv.s}%`,
                top: `${100 - hsv.v}%`,
              }}
          ...
          <label className="kaisa-colorpicker__hue">
            <span className="kaisa-colorpicker__hue-label">Hue</span>
            <input
              type="range"
              min={0}
              max={360}
          ...
          <div className="kaisa-colorpicker__preview-row">
            <span
              className="kaisa-colorpicker__preview"
              style={{backgroundColor: selected ?? hsvToHex(hsv)}}
            />
            <input
              type="text"
              className="kaisa-input kaisa-input--sm kaisa-colorpicker__hex"
          ...
          {showPresets ? (
            <div className="kaisa-colorpicker__presets">
              {presets.map((color) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    className={joinClasses(
                      'kaisa-colorpicker__preset',
                      isActive && 'kaisa-colorpicker__preset--active',
                    )}
