...
export type Ex3ColorpickerSize = 'sm' | 'md' | 'lg';

export type Ex3ColorpickerProps = {
  value?: string | null;
...
export function Ex3Colorpicker({
  value,
  defaultValue = '#1a1a18',
  onChange,
  ...
  return (
    <div
      ...
      className={joinClasses(
        'ex3k-colorpicker',
        `ex3k-colorpicker--${uiSize}`,
        open && 'ex3k-colorpicker--open',
        disabled && 'ex3k-colorpicker--disabled',
        readOnly && 'ex3k-colorpicker--readonly',
        invalid && 'ex3k-colorpicker--invalid',
        className,
      )}
    >
      <div className="ex3k-colorpicker__control">
        <button
          type="button"
          className="ex3k-colorpicker__swatch"
        ...
        <div className="ex3k-colorpicker__actions">
          {clearable && selected && !disabled && !readOnly ? (
            <button
              type="button"
              className="ex3k-colorpicker__clear"
              onClick={handleClear}
              aria-label="색상 지우기"
            >
              ×
          ...
          <button
            type="button"
            className="ex3k-colorpicker__trigger"
            onClick={() => (open ? closePopover() : openPopover())}
            disabled={disabled || readOnly}
      ...
      {open ? (
        <div
          id={`${inputId}-palette`}
          className="ex3k-colorpicker__popover"
          role="dialog"
          aria-label="색상 선택"
        >
          <div
            ref={svRef}
            className="ex3k-colorpicker__sv"
            style={{backgroundColor: hsvToHex({h: hsv.h, s: 100, v: 100})}}
            onPointerDown={handleSvPointerDown}
          >
            <div className="ex3k-colorpicker__sv-white" />
            <div className="ex3k-colorpicker__sv-black" />
            <span
              className="ex3k-colorpicker__cursor"
              style={{
                left: `${hsv.s}%`,
                top: `${100 - hsv.v}%`,
              }}
          ...
          <label className="ex3k-colorpicker__hue">
            <span className="ex3k-colorpicker__hue-label">Hue</span>
            <input
              type="range"
              min={0}
              max={360}
          ...
          <div className="ex3k-colorpicker__preview-row">
            <span
              className="ex3k-colorpicker__preview"
              style={{backgroundColor: selected ?? hsvToHex(hsv)}}
            />
            <input
              type="text"
              className="ex3k-input ex3k-input--sm ex3k-colorpicker__hex"
          ...
          {showPresets ? (
            <div className="ex3k-colorpicker__presets">
              {presets.map((color) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    className={joinClasses(
                      'ex3k-colorpicker__preset',
                      isActive && 'ex3k-colorpicker__preset--active',
                    )}
