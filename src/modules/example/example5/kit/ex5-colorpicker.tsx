...
export type Ex5ColorpickerSize = 'sm' | 'md' | 'lg';

export type Ex5ColorpickerProps = {
  value?: string | null;
...
export function Ex5Colorpicker({
  value,
  defaultValue = '#1a1a18',
  onChange,
  ...
  return (
    <div
      ...
      className={joinClasses(
        'ex5k-colorpicker',
        `ex5k-colorpicker--${uiSize}`,
        open && 'ex5k-colorpicker--open',
        disabled && 'ex5k-colorpicker--disabled',
        readOnly && 'ex5k-colorpicker--readonly',
        invalid && 'ex5k-colorpicker--invalid',
        className,
      )}
    >
      <div className="ex5k-colorpicker__control">
        <button
          type="button"
          className="ex5k-colorpicker__swatch"
        ...
        <div className="ex5k-colorpicker__actions">
          {clearable && selected && !disabled && !readOnly ? (
            <button
              type="button"
              className="ex5k-colorpicker__clear"
              onClick={handleClear}
              aria-label="색상 지우기"
            >
              ×
          ...
          <button
            type="button"
            className="ex5k-colorpicker__trigger"
            onClick={() => (open ? closePopover() : openPopover())}
            disabled={disabled || readOnly}
      ...
      {open ? (
        <div
          id={`${inputId}-palette`}
          className="ex5k-colorpicker__popover"
          role="dialog"
          aria-label="색상 선택"
        >
          <div
            ref={svRef}
            className="ex5k-colorpicker__sv"
            style={{backgroundColor: hsvToHex({h: hsv.h, s: 100, v: 100})}}
            onPointerDown={handleSvPointerDown}
          >
            <div className="ex5k-colorpicker__sv-white" />
            <div className="ex5k-colorpicker__sv-black" />
            <span
              className="ex5k-colorpicker__cursor"
              style={{
                left: `${hsv.s}%`,
                top: `${100 - hsv.v}%`,
              }}
          ...
          <label className="ex5k-colorpicker__hue">
            <span className="ex5k-colorpicker__hue-label">Hue</span>
            <input
              type="range"
              min={0}
              max={360}
          ...
          <div className="ex5k-colorpicker__preview-row">
            <span
              className="ex5k-colorpicker__preview"
              style={{backgroundColor: selected ?? hsvToHex(hsv)}}
            />
            <input
              type="text"
              className="ex5k-input ex5k-input--sm ex5k-colorpicker__hex"
          ...
          {showPresets ? (
            <div className="ex5k-colorpicker__presets">
              {presets.map((color) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    className={joinClasses(
                      'ex5k-colorpicker__preset',
                      isActive && 'ex5k-colorpicker__preset--active',
                    )}
